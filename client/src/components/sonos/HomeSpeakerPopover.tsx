...
// ── HomeSpeakerPopover ────────────────────────────────────────────────────────
//
// Thin wrapper that renders SpeakerCard for each speaker/group exactly like the
// Playing page does, with `showVolume={false}` so the volume slider is hidden
// (Home has its own dedicated volume popover). Queue, group panel, browse
// fallback, playback controls, and "See full queue" all come from the shared
// SpeakerCard → UnifiedPlaybackCard component — no custom accordions here.

export function HomeSpeakerPopover({
  open,
  onClose,
  triggerRef,
  borderColor,
}: HomeSpeakerPopoverProps) {
  ...
  return (
    ...
    >
      <div className="px-3 py-3">
        ...
        {/* Speaker rows — identical to the Playing page, minus volume */}
        {!isLoading && renderItems.length > 0 && (
          <ul className="space-y-2" role="list">
            {renderItems.map(item => {
              ...
              return (
                <li key={item.coordinator.speakerName}>
                  <SpeakerCard
                    type="group"
