...
test('Settings Music section shows Connect Spotify link when Spotify is disconnected', async ({ page }) => {
  ...
  // The Spotify section heading — the disconnected state also renders the
  // setup-wizard headings ("Step 1 — …"), so match exactly and take the first
  await expect(page.getByRole('heading', { name: 'Spotify', exact: true }).first()).toBeVisible()
